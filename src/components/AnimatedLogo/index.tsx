  ...
  const content = (
    <div className={`flex flex-col items-center gap-4 ${className}`}>
      <style>{`
        ...
        @keyframes gLeafUnfurl {
          0%, 30% { transform: scale(0) rotate(-20deg); opacity: 0; }
        ...
        @keyframes gPetalGlow {
          0%, 100% { filter: drop-shadow(0 0 3px rgba(222,129,141,0.15)); }
          50% { filter: drop-shadow(0 0 10px rgba(222,129,141,0.35)); }
        }
      `}</style>
      ...
      >
        {/* Stem growing up from the "soil" line */}
        <svg
          viewBox="0 0 100 100"
          className="absolute inset-0"
          style={{ width: '100%', height: '100%', animation: 'gSway 3.2s ease-in-out infinite', transformOrigin: '50% 100%' }}
        >
          <path
            d="M50 95 C 50 70, 48 55, 50 30"
            fill="none"
            stroke="#8bc48a"
            strokeWidth="3"
            strokeLinecap="round"
            strokeDasharray="80"
            style={{ animation: 'gStemGrow 1.8s ease-out forwards' }}
          />
          {/* Left leaf */}
          <path
            d="M50 62 C 38 60, 30 52, 30 44 C 42 44, 50 52, 50 62 Z"
            fill="#9ed49a"
            style={{ transformOrigin: '50px 55px', animation: 'gLeafUnfurl 1.8s ease-out 0.3s forwards' }}
          />
          {/* Right leaf */}
          <path
            d="M50 48 C 62 46, 70 38, 70 30 C 58 30, 50 38, 50 48 Z"
            fill="#8bc48a"
            style={{ transformOrigin: '50px 40px', animation: 'gLeafUnfurl 1.8s ease-out 0.5s forwards' }}
          />
        </svg>

        {/* Logo blooming at the top of the stem */}
        <svg
          ...
          style={{
            width: size,
            height: size,
            marginBottom: size * 0.62,
            animation: 'gBloom 1.8s cubic-bezier(0.34,1.56,0.64,1) 0.6s forwards, gPetalGlow 2.5s ease-in-out 2.4s infinite',
            opacity: 0,
            position: 'relative',
            zIndex: 1,
          }}
        >
          <rect
            x="1" y="1" width="254" height="254" rx="30"
            fill="none"
            stroke="#de818d"
            strokeWidth="2"
          />
