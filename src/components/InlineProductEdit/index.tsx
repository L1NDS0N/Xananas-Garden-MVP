...
import { X, FloppyDisk, Upload, Trash, CaretUp, CaretDown } from 'phosphor-react';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <MultiSelectDropdown
                ...
                selectedIds={Array.from(new Set([form.categoryId, ...form.categoryIds].filter(Boolean)))}
                onChange={ids => setForm(prev => ({ ...prev, categoryId: ids[0] || '', categoryIds: ids }))}
          ...
          {paymentMethods.filter(m => m.active).length > 0 && (
            <div>
              ...
              <MultiSelectDropdown
                ...
                selectedIds={form.paymentMethodIds}
                onChange={ids => setForm(prev => ({ ...prev, paymentMethodIds: ids }))}
                placeholder="Nenhuma marcada = aceita todas as formas ativas"
              />
            </div>
          )}

          {form.paymentMethodIds.some(id => paymentMethods.find(m => m.id === id)?.key === 'card') && (
            <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4 bg-gray-50 rounded-lg p-3 border border-gray-100">
              <div className="w-full sm:w-32">
                ...
                <select value={form.maxInstallments}
                  onChange={e => setForm(prev => ({ ...prev, maxInstallments: parseInt(e.target.value) }))}
              ...
              <div className="flex items-center gap-2 py-2">
                <input type="checkbox" checked={form.installmentInterest}
                  onChange={e => setForm(prev => ({ ...prev, installmentInterest: e.target.checked }))}
                  className="w-4 h-4 text-[#de818d] rounded" />
