        ...
        <div className="leading-relaxed">
          {
            items.map((item, index) => {
              return (
                <Link href={item.href ?? '#'} key={index} className="flex my-2 border-solid border-b border-transparent hover:border-zinc-300"
                  onClick={() => onSelect && onSelect(item)}>{item.name}</Link>
              )
            })
          }
        </div>
