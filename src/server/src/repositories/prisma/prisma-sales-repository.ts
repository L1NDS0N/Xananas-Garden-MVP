...
export class PrismaSalesRepository implements ISalesRepository {
  async create({ items, discount, paymentType, notes, userId, clientId, couponId, couponCode, couponDiscount }: CreateSaleData): Promise<string> {
    ...
    // Decrement stock + record history for each item — a variant's own stock is decremented
    // instead of the parent product's when the item was sold as a specific subproduto.
    for (const item of items) {
      if (item.variantId) {
        const variant = await prisma.productVariant.findUnique({ where: { id: item.variantId } });
        if (!variant) continue;
        const previousAmount = variant.amount || 0;
