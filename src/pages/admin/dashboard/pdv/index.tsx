...
interface CartItem {
  product: Product;
  quantity: number;
  /** Subproduto selecionado (muda, enxerto, cor, etc.) — usa o preço/estoque próprios da variação */
  variantId?: string;
}

/** Ativas apenas — o mesmo filtro usado para decidir se um produto exige escolher uma variação */
function activeVariantsOf(product: Product): ProductVariant[] {
  return (product.variants || []).filter(v => v.active);
}

function itemVariant(item: CartItem): ProductVariant | null {
  if (!item.variantId) return null;
  return item.product.variants?.find(v => v.id === item.variantId) || null;
}

function itemPrice(item: CartItem): number {
  const v = itemVariant(item);
  if (v) return v.price ?? item.product.price;
...
function itemLabel(item: CartItem): string {
  const v = itemVariant(item);
  return v ? `${item.product.name} — ${v.name}` : item.product.name;
}
...
const PDV: React.FC = () => {
  ...
  // A product with active variants is only sellable through one of them — its own stock is ignored.
  const availableProducts = allProducts.filter((p: any) => {
    if (p.published === false) return false;
    const variants = activeVariantsOf(p);
    if (variants.length > 0) {
      return variants.some(v => {
        const inCart = cart.find(c => c.product.id === p.id && c.variantId === v.id);
        const remaining = inCart ? v.amount - inCart.quantity : v.amount;
        return remaining > 0;
      });
    }
    if (p.amount <= 0) return false;
    const inCart = cart.find(c => c.product.id === p.id && !c.variantId);
  ...
  // Adds a specific variant (or the base product, when it has none) to the cart, checking that variant's own stock
  const addLineToCart = (product: Product, variantId?: string) => {
    const stock = variantId ? (product.variants?.find(v => v.id === variantId)?.amount ?? 0) : product.amount;
    const existing = cart.find(item => item.product.id === product.id && item.variantId === variantId);
    if (existing) {
      ...
      setCart(prev => [...prev, { product, quantity: 1, variantId }]);
    }
  };
  ...
  const updateQuantity = (productId: string, delta: number, variantId?: string) => {
    setCart(prev => {
      ...
      if (newQty <= 0) return prev.filter(i => !(i.product.id === productId && i.variantId === variantId));
      if (newQty > itemStock(item)) {
        toast('Estoque insuficiente', 'warning');
        return prev;
      }
      return prev.map(i => (i.product.id === productId && i.variantId === variantId) ? { ...i, quantity: newQty } : i);
    });
  };
  ...
  const buildReceiptWaUrl = (sale: any) => {
    const items = (sale.items || []).map((i: any) => {
      const name = i.variantLabel ? `${i.product?.name || 'Produto'} — ${i.variantLabel}` : (i.product?.name || 'Produto');
      return `• ${i.quantity}x ${name} — ${formatPrice(i.unitPrice * i.quantity)}`;
    }).join('\n');
    ...
    try {
      const res = await api.post('/sales', {
        items: cart.map(item => ({
          ...
          variantLabel: itemVariant(item)?.name,
        })),
      ...
      await api.post('/cash-flow', {
        type: 'entry',
        description: `Venda #${res.data.id?.slice(0, 8) || 'PDV'} - ${cart.map(i => `${i.quantity}x ${itemLabel(i)}`).join(', ')}`,
        amount: finalTotal,
        userId: user?.id,
      });
  ...
  return (
    <AuthGuard>
      <DefaultPage title="Ponto de Venda">
                        ...
                        {filteredProducts.map(product => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {hasVariants ? (
                                <p className="text-[10px] text-[#de818d] font-medium">{variants.length} variações</p>
                              ) : (
                                <>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-4 sticky top-20">
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto mb-4">
                        {cart.length === 0 ? (
                          ...
                          cart.map(item => (
                            <div key={`${item.product.id}-${item.variantId || ''}`} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium text-gray-800 truncate">{itemLabel(item)}</p>
                                <p className="text-[10px] text-gray-400">{formatPrice(itemPrice(item))} un.</p>
                              </div>
        ...
        {/* Variant picker — shown when adding a product that has subprodutos (muda, enxerto, cor, etc.) */}
        {variantPickerProduct && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="absolute inset-0 bg-black/50" onClick={() => setVariantPickerProduct(null)} />
            <div className="relative bg-white rounded-2xl p-5 max-w-sm w-full mx-4 shadow-xl">
              <h3 className="text-lg font-bold text-gray-800 mb-1">{variantPickerProduct.name}</h3>
              <p className="text-xs text-gray-400 mb-3">Escolha a variação</p>
              <div className="space-y-2 max-h-72 overflow-y-auto">
                {activeVariantsOf(variantPickerProduct).map(v => {
                  const inCart = cart.find(c => c.product.id === variantPickerProduct.id && c.variantId === v.id);
                  const remaining = v.amount - (inCart?.quantity || 0);
                  return (
                    <button key={v.id} disabled={remaining <= 0}
                      onClick={() => { addLineToCart(variantPickerProduct, v.id); setVariantPickerProduct(null); }}
                      className="w-full flex items-center justify-between gap-2 p-3 border border-gray-200 rounded-lg hover:border-[#de818d] hover:bg-pink-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-left">
                      <div>
                        <p className="text-sm font-medium text-gray-800">{v.name}</p>
                        <p className="text-[10px] text-gray-400">{remaining > 0 ? `Estoque: ${remaining}` : 'Indisponível'}</p>
                      </div>
                      <span className="text-sm font-bold text-[#de818d]">{formatPrice(v.price ?? variantPickerProduct.price)}</span>
                    </button>
                  );
                })}
              </div>
              <button onClick={() => setVariantPickerProduct(null)}
                className="w-full mt-3 py-2 text-sm text-gray-500 hover:text-gray-700">Cancelar</button>
            </div>
          </div>
        )}
              ...
              <div className="space-y-1 mb-4 max-h-40 overflow-y-auto">
                {cart.map(item => (
                  <div key={`${item.product.id}-${item.variantId || ''}`} className="flex justify-between text-sm">
                    <span className="text-gray-600">{item.quantity}x {itemLabel(item)}</span>
                    <span className="font-medium">{formatPrice(itemPrice(item) * item.quantity)}</span>
                  </div>
                ))}
              </div>
